<div class="clear-block comment<?php print ($comment->new) ? ' comment-new' : ''; print(isset($comment->status) && $comment->status == COMMENT_NOT_PUBLISHED) ? ' comment-unpublished' : ''; if (isset($author_comment)) print ' author'; print ' '. $zebra; ?>">
  ...
  <div class="comment-content">
    ...
    <div class="content">
     ...
     <?php if ($submitted): ?>
        <span class="username"><?php print t('Posted by ');?><?php print theme('username', $comment); ?></span> <span class="date"><?php print t(' !date ago', array( '!date' => format_interval(time() - $comment->timestamp))); ?></span>
      <?php endif; ?>
    <?php if ($links): ?>
